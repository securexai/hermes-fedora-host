...
module_hash=37643c3428ee847e82643056142d1ff5702d7249ed8046e71326c1e01b00a6b8
base_hash=bf58d5f5ae23502d844f70c153ef5366c31bf16ac9c77f548b94f3cf1aea1949
merged_hash=84e62bca8dbd0439593c7fbf44f795a9f1a3d5b461f5a174baf35c9346678360
exe=/usr/lib/systemd/systemd-tpm2-setup
policy=/etc/selinux/targeted/policy/policy.35
backup=/var/lib/hermes-selinux-trial-20260909-v2
fail() {
  printf '%s\n' "$*" >&2
  exit 1
}
...
[[ $# == 2 && $1 == --approved-policy-install ]] || {
  printf '%s\n' 'Usage: bash install-reviewed-candidate-v2.sh --approved-policy-install /absolute/candidate.pp' >&2
  exit 64
}
